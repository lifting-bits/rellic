    ...
    switch (type->getTypeID()) {
      ...
      case llvm::Type::StructTyID: {
        ...
        if (!decl) {
          ...
          auto sname{
              strct->isLiteral()
                  ? ("literal_struct_" + std::to_string(num_literal_structs++))
                  : strct->getName().str()};
          if (sname.empty()) {
    ...
    switch (l_type->getTypeID()) {
      ...
      default: {
        ...
        if (l_type->isVectorTy()) {
          ...
          THROW() << "Unknown LLVM constant type: "
...
void IRToASTVisitor::VisitGlobalVar(llvm::GlobalVariable &gvar) {
  ...
  var = ast.CreateVarDecl(tudecl, expr_gen.GetQualType(type), name);
  // Add to translation unit
  tudecl->addDecl(var);
  // Create an initalizer literal
  if (gvar.hasInitializer()) {
    clang::cast<clang::VarDecl>(var)->setInit(
        expr_gen.CreateConstantExpr(gvar.getInitializer()));
  }
}
