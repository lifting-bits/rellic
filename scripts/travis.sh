...
common_build() {
  ...
  if [ "${llvm_version:0:1}" == "3" ] ; then
    ...
    ( cd build && make -j `nproc` && pyenv local 3.7.1 && make test) > "${log_file}" 2>&1 &
  else
    printf " i Clang static analyzer enabled\n"
    ( cd build && scan-build --show-description make -j `GetProcessorCount` && pyenv local 3.7.1 && make test) > "${log_file}" 2>&1 &
  fi
